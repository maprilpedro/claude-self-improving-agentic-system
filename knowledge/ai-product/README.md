...
| [A Multi-Team Agent Test Measures Whatever Is Least Controlled — Usually Access, Not the Agent](a-multi-team-agent-test-measures-whatever-is-least-controlled.md) | An agent answers only after identity, entitlement, provisioning, manifest, skill load and API config have all held, but a multi-team test attributes every failure to the last link. So it measures the least-controlled link, which in enterprise is access — the only per-person, per-environment layer. Run the chain yourself on the exact target first, pre-clear that link, and say which link each failure sat on… |
...
| [The Real Cost of a Platform Migration Lands as Rework in Teams Nobody Consulted](the-real-cost-of-a-platform-migration-lands-as-rework-in-teams-nobody-consulted.md) | When the execution substrate changes, downstream teams inherit work nobody estimated — threat models, removed renderers. It is announced in a subordinate clause in a Slack thread and lands on no roadmap. The gap between the port date and the readiness date is made of this. (2026-07-13, 2 obs) |
