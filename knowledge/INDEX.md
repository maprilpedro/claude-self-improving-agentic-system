...
| `rhetoric/` | **On-demand only.** Checkable rhetoric rules for drafting replies. Read `rhetoric/README.md` (router: situation → rules) when Pedro asks « check rhéto / une règle s'applique ? ». Never auto-load. |
...
| Ferry, *12 leçons de rhétorique pour prendre le pouvoir* | 2026-07-15 | Book (full) → `rhetoric/ferry.md` | 37 checkable argument-structure rules, one per drawer, voice-tagged charpente/ornement. The most operational of the three rhetoric sources. On-demand retrieval via `rhetoric/README.md`. |
| Viktorovitch, *Le pouvoir rhétorique* (Seuil 2021) | 2026-07-15 | Book **(49-page preview only)** → `rhetoric/viktorovitch.md` | Definition + framing (convince AND decrypt) + the real TOC as a technique map. ⚠️ Body absent from the source file; attack/defense/answering-questions techniques still to extract if the full PDF is supplied. |
| Aristote, *La Rhétorique* (`.doc`) | 2026-07-15 | **Course-notes digest, not the primary text** → `rhetoric/classical-scaffold.md` | The 3 genres, 4 parts, argument types, enthymeme, argument-bearing figures (prolepse, présupposition, qualification), the 3 principles (non-paraphrase / fermeture / transfert). Scaffold + vocabulary, not tactics. |
...
| *IA et Stratégie*, « Le modèle SaaS est brisé » (Claude Force) | 2026-09-02 | Video (vault note) | Three-storey model (conversation / intelligence / governance), the three-question vendor test, rent the intelligence but own the learning loop, the screen as porthole. Applied to Adobe/Coworker/AEM in patterns/ |
...
| **Checking a rhetorical move in a draft (on demand only)** | `rhetoric/README.md` (situation → rules) | `rhetoric/ferry.md`, `rhetoric/classical-scaffold.md`. Trigger: « check rhéto / une règle s'applique ? » |
