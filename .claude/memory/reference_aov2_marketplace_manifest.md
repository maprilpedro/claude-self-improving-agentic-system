...
⚠️ **Unverified 2026-07-17:** whether One AEM MCP is still wired into `aem-aia` as the baseline fallback (Reasor PR `ao#5388`). It is not in the `plugins:` list; it may sit elsewhere in the file. **Check before repeating the "aem-aia = extensions + One AEM MCP baseline" line.**
