...
> ### 🔴🔑 2026-08-06 — RUBIN SYNC (held, ~22 min). **THE AXIS HE DECIDED ON THAT MORNING IS THE ONE AXIS THE REPORTING SUBSTRATE CANNOT SEE**
>
> Source: transcript `2026/AEM Agents Intelligence/Rubin/20260806 - AEM Rubin Sync.md`, read in full 2026-08-07. **Named speakers throughout, no room mics — attribution is clean, unusually for this lane.** Room: Pedro, Yanira Castaneda, **Angela Han** (Rubin owner), **Karthik Penikalapati** (Rubin tech lead), **Venkatesh Kunda** (Sr Analytics Engineer, NY). ⚠️ **Venkatesh is NOT a new stakeholder** — already in [[project_adobe_org]] under Angela since the April org read. Checked before writing 🆕, per the Huong Vu rule in [[reference_transcript_glossary]].
>
> **This is the meeting behind Pedro's own 08-06 09:34 line that he had *"restarted the dashboard discussion with the Rubin team"*, and it is the one-month follow-up to the 07-06 Angela meeting** (shard `..._ARCHIVE_2026-W28a.md`), whose two live threads were the **SKU/external-customer split** and the **daily digest as a prototyping surface**. Both were picked up on 08-06. **Neither is closed.**
>
> **🔴🔑 THE FINDING, AND IT IS A COLLISION BETWEEN TWO OF HIS OWN 08-06 DECISIONS.** That morning PR15 rooted the agent axis on the **plugin**, on the evidence that the plugin is declared on 88 of 88 and never forks while the name token is inferred, covers 67 of 88 and **disagrees with the declared `domain:` on 14 of the 18 skills carrying both**. That afternoon he asked Rubin to report by plugin and **Rubin does not log it.** Karthik: *"we still did not **get that logging data**, but I think we can start with the skill grouping."* Venkatesh: *"plugins is something I mean, I think we might have to think about it at some point later."* **The only grouping Rubin offers is the skill name — precisely the axis PR15 disqualified hours earlier.** Pedro named the fallback himself in the room without flagging the conflict: *"I would then fall back to reporting on group of skills based on some of their namings… I would prefer the plugin, which is again the container of the skills."*
> 🔑 **So the ownership decision and the measurement substrate now disagree, and the substrate wins by default** — every published agent-cut number would silently ride the axis the decision record calls unreliable. **Two honest exits: get `plugin` into Rubin's logging, or publish on the name and footnote the 14-of-18 disagreement.** This is the sharp new instance of [[A Packaging Unit Is a Customer Taxonomy — Ownership Needs Its Own Record]] and it extends the entry: choosing the axis is not the end, **the telemetry has to carry it.**
>
> **🟢 WHAT HE GOT.** Rubin builds a **digest per skill-group** from a grouping map Pedro supplies, then a dashboard — Karthik: *"we will just take the grouping Pedro shares… create a digest for each one… which will be create a dashboard, right? So that should be out-of-the-box."* Plus **early access to an unreleased Report Builder**, week of 08-10, PR merging the week of 08-04. Venkatesh: *"we did not kind of market that product yet, so I think you'll be one of the very first users."* Reports render HTML, carry a subscriber list, and **others can clone and customise them** — which is the distribution property the three Workday reviewers asked for.
> ⚠️ **The grouping map he owes is the same object as the seven open agent-map decisions — 51 of 88 skills have no agent.** He cannot send it complete before 08-10.
>
> **🔑 KARTHIK'S DESIGN POINT IS MEASUREMENT INTEGRITY, AND IT ARRIVES FROM OUTSIDE.** The Report Builder exists because ad-hoc querying is non-deterministic: *"if you go and ask like an investigate, it **might assume your SQL in a different way when different people ask the same question**. This way your report is grounded to the same SQL query"* — the same grounded answer every day. **A platform team shipped a product against the exact failure Pedro hit twice in two weeks** (07-24, three instruments each producing a misleading headline until scope/window/basis were pinned; 08-06/07, the published audit numbers moving 88→98 the same day so the linked report contradicts the message). Logged as a refinement to [[Success Definitions Must Be Agreed Before Metrics Are Scaled]] — the entry covered definitions across *teams*; this adds definitions across *askers and time*.
>
> **🔴 SKU vs TRY-BEFORE-YOU-BUY CANNOT BE SPLIT, AND THE BLOCKER IS OLDER THAN THIS MEETING.** Pedro asked whether the reports can segregate paying-SKU orgs from TBYB. Rubin's provisioning data is AEP-side and nobody could say whether AEM provisions the same way — *"we just have to confirm."* Karthik: *"if there is an entitlement, right, then it becomes so easy."* **Yanira supplied the harder fact: Andre (DAS team) has already said there is no quick indicator** — *"it's a challenge to check by the SKU or the try before you buy customers because there isn't like a quick indicator… I don't think it's something that's easy on his side."* She will surface it on Slack. **This is the 07-06 SKU follow-up, still open at one month, and it now blocks Namita's TBYB cohort ordering.**
>
> **🟢🔑 ANGELA REFUSED THE SHORTCUT, CORRECTLY, AND PEDRO TOOK IT WELL.** He asked whether Rubin could be **side-loaded** with extra per-org information. Angela: *"I want to make sure that what we're building is **sustainable**. Because **are you going to side load them every week?**"* Pedro conceded on the spot — *"Yep, makes sense, Angela. So we'll have to identify the **root source** of that provisioning information, like an API or else, in order to make it sustainable."* **→ The deliverable is a provisioning API, not a spreadsheet.** ⚠️ **And note what this is NOT:** she objected to the **cadence**, not to the capability. **It is adjacent evidence on [[H-009]]'s ingress question and it cuts mildly AGAINST Hemanta Gupta's walled-garden reading — but it is not an answer and must not be banked as one.** Venkatesh's interim hack stands: one digest filtered to the TBYB org list, one to the paid list.
>
> **🔴🔑 THE MISS WORTH RECORDING: TWO OPEN QUESTIONS WERE IN THE ROOM WITH BOTH PEOPLE NAMED AS THEIR VERIFIERS, AND NEITHER WAS ASKED.** The Status board carried (1) Ramkesh Meena's *"the CX Enterprise Coworker tab appears to be available only for GA skills"*, explicitly routed *"confirm with Angela Han or Karthik Penikalapati before saying it in the room"* — **and 08-10 is when he needs it**; and (2) Hemanta's no-ingress finding, routed *"Angela Han or Manas"*. **Angela and Karthik were both present for 22 minutes.** This is a retrieval failure, not a knowledge gap — both questions were written down, dated, and named the right people ([[feedback_read_kr_before_drafting]], and the P6 retrieval rule in CLAUDE.md). **The mechanism to fix is cheap: read the open watches for a person before the meeting with that person.**
>
> **⚖️ H-009 — DELIBERATELY NOT LOGGED AS EVIDENCE.** The 2026-08-03 System Review restricted this hypothesis to three signals: an AOv1-decommission DACI from Meschberger, Marth's veto being lifted, or the IAM+RBAC answer on Databricks access. **None of the three moved here.** Angela's engagement is real but it is the same shape already over-counted once — recorded in this block and kept out of `hypotheses/active.md` on purpose.
>
> **🔎 KNOWLEDGE (P6):** [[A Packaging Unit Is a Customer Taxonomy — Ownership Needs Its Own Record]] (**refined — the telemetry-carries-the-axis corollary**), [[Success Definitions Must Be Agreed Before Metrics Are Scaled]] (**refined — grounded query vs ad-hoc query**), [[Say the Sentence That Obliges — the Hedge Transfers the Ask]] (⚠️ **candidate only, deliberately not counted** — the GA date did carry the ask and produced early access to an unreleased tool, but the authority sat in a calendar date rather than an object he owned, and the 08-03 review caught this hypothesis being padded once already), [[Metric Definition Ownership — PM Validates, Reporting Track Owner Implements]] (Pedro supplies the grouping, Rubin implements — the pattern holding cleanly).
