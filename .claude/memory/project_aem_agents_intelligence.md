...
> ## ▶️ RESUME HERE — left off 2026-06-01
>
...
>
> ### 🔴 2026-06-01 (#2) — CX Coworker = AOv2 customer-facing; GA June 11; AOv2-timeline call being scheduled
> **Identity locked (Bertrand's own words, May 1 #aem-agent-experience-production):** *"AOv2 is AO+AI Assistant in one box - and the foundation for the CX Coworker - so maybe keep the AOv2 label internal, and have integrate with CX Coworker customer facing terminology?"* So the relationship is **nested, not lateral**: **CX Coworker** (customer-facing product) → built on **AOv2** = **AO 2.0** (harness/runtime) **+ AIA 2.0** (AI Assistant 2.0 / the chat UI). Bertrand June 1: AO 2.0 "has its own UI now, **different from AIA 2.0**" = two UI lineages diverging (coherence risk = Pedro's consistency lane). **Naming is exec-controlled:** Amit pulled "CX Coworker" from BV demos Apr 15, "Coworker"→"Assistant" in header (Guliz, #experience-workspace).
> - **🔴 CX Coworker GA June 11** (Bertrand "heard it", email June 1 — NOT confirmed in-thread; first question for the call).
