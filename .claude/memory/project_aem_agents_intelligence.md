...
>
> ### 🔬🔑 2026-08-15 EVENING — HE STOPPED ASKING AND REPRODUCED IT INSTEAD, AND NINETY MINUTES CLOSED FOUR QUESTIONS THAT WERE QUEUED FOR AO OPS
>
> **The move worth keeping: three of the four open questions about the bug bash environment were sitting in `watches.md` addressed to other people, with answers due Monday at the earliest. He answered them himself, from a browser, between 18:40 and 21:10.** [[Say the Sentence That Obliges — the Hedge Transfers the Ask]] — the authority ended up in a reproduction rather than in anyone's opinion. ⏳ **Candidate for the 2026-09-01 review, deliberately NOT parked now** (cap is 7 of 8, closed until then): *reach for the reproduction before the escalation — an ops question you can reproduce is not an ops question.*
>
> **✅ THE STAGE ROUTE IS DEAD, AND IT IS PROVEN, NOT ARGUED.** Feeding `/aem-assets-discovery` the real Showcase stage-author URL from a stage Coworker session returned **`401 invalid OAuth token`** with the reason in clear: *"the AEM environments at `author-p149891-*` belong to a **different IMS org** than the one your session is authenticated against (`3C876885616965D90A494029@AdobeOrg`)."* **The last alternative was tested too** — `AEM Sites Engineering` shows `Experience Manager Assets` in its stage launcher but returns `403 "Profile is not valid" (403025)` on Cloud Manager and `User is not provisioned` on Assets, with **no Admin Console in that org's launcher**, so he cannot self-provision. → **The bash runs on the prod shell.**
>
> **✅ AND THE MERGED PLUGIN SET WORKS — FIRST REAL EXERCISE OF IT, BY HIM, ALONE.** On stage org `CM Gandalf`, typing `/aem` listed the skills, and `/aem-cloudmanager-program-management` reported *"made 1 API call"* and returned **13 real programs (9 Ready, 4 Failed)** with CSV export. **Manifest → skill load → auth → `cm` api_config → live API.** That partly closes Lenard's 08-14 *"I'm not sure how to test the rest of the plugins"*, which nobody had owned. ⚠️ An empty `/aem-assets-discovery` on the same org was **not** a bug — program `135659` has no environments provisioned, so the empty answer was correct and a second skill corroborated the first. 🔑 The skill also offers a **manual author-URL fallback**, which is a live workaround for **Cole Connelly's instance-picker ask**.
>
> **🔴🔑 THE FINDING THAT MATTERS MOST FOR TUESDAY — EVERY WALL WAS AN ENTITLEMENT, FIVE FOR FIVE, NEVER A SKILL AND NEVER A CONFIG.** `Failed to authenticate with Adobe ID` on a Showcase author **while being org admin** · `401` cross-org · `no environments provisioned` · `403025` · `User is not provisioned`. **Admin rights are not access — you reach an AEM environment only through one of its product profiles.** → **The dominant failure mode for ~30 bug bashers will be product profiles, and if it is not fixed in advance the bash measures provisioning and reports it as agent quality, with the 08-20 confidence declaration inheriting the error.**
>
> **📍 THE OBJECTS YANIRA HAS BEEN BLOCKED ON SINCE 08-15 03:32 NOW EXIST.** AEM Showcase is an **IMS org** containing **several** Cloud Manager programs, which is why "target = Showcase" never settled anything. The live one is **`AEM Showcase Program` 149891**, prod-author **`1546481`** / stage-author **`1546482`**, both with eight product profiles at 256-270 entitled users. Its stage-author carries real content — `aemshowcase`, `Frescopa`, `Company` in Sites and **19 asset folders** including `MegaFormsDemo`. **`AEM Showcase EDA CxCowork-2` (210760 / 2207539) is a shell** — zero assets, five stub trees, two of them Workfront-shaped. **Her three answers: `149891` · `1546481` · `AEM Users` + `AEM Sites Content Managers` (+ `AEM Assets Collaborator Users`).**
>
> **📤 HE POSTED THE FINDING AT 21:23** in the Carsten group DM `C0BPSMZBRPB` → https://adobe.enterprise.slack.com/archives/C0BPSMZBRPB/p1786821819886959, with three ranked alternatives. **No replies at 21:30.** ⚠️ **Three gaps flagged to him:** the **401 is not in the post** (he wrote *"I believe"* and *"if you want to confirm"* while holding a verbatim reproduction), the **ranking is inverted** (he put "find a proper Stage Org" first while writing *"I dont know any"*, an hour after ruling out the obvious candidate), and there is **no date** on a bash starting Tuesday — unlike the 08-13 entitlement mandate that moved five teams in eight hours because it carried a field, a snippet, a deadline and an audit.
>
> **🟢🔑 HIS OWN THIRD OPTION IS THE BEST ONE ON THE TABLE — A SHOWCASE-SCOPED PROD MANIFEST**, carrying the plugin list and its own `api_configs` copied from dev/stage, instead of porting into the shared `prod/cx-coworker.yaml`. **The precedent is `cx-coworker-lloyds.yaml` and `cx-coworker-prada.yaml`**, which Lenard's 08-14 PR reached by inheritance. **It dissolves Ian Reasor's stated objection outright** (*"concerned about potential regressions for the AEP team that is already using the prod manifest in prod"*) and voids the CODEOWNERS *"high blast radius"* warning, turning Monday's ask from a risk-budget request into a one-day yes. ⚠️ **Unknown for Lenard or Gerald: how an org is bound to a manifest.** ⚠️ Counter-argument is his own 08-13 line about staying close to the GA setup, **weaker here because both blocks would be copies of what ships**.
>
