...
> **Source:** `Meeting Notes/Bertrand 1 1/20260604 - Bertrand Pedro 1 1.md` (~25 min, very garbled Otter FR — Pedro reviewed + confirmed 5 attribution/name corrections before banking). Held morning of the Thursday before Friday's arch session.
...
> - **🆕 Sergei AOv2 cross-agent demo (~2 weeks ago, Friday-PM demo slot)** — RT-CDP agent detects 5000 customers about to cancel Photoshop (real Bicom data) → event emitted → AJO agent auto-builds a targeted campaign with tailored content. **Stops there.** Bertrand: *"dommage si on pouvait continuer avec les agents AEM"* = explicit cross-agent plug-in opening for Pedro's AEM agents. (Watch the Friday-PM demo channel — Bertrand attends remotely, recommends Pedro do too.)
> - **🆕 Next-week workshop presentation — Pedro takes over Peter's slot.** 56 people, mixed knowledge levels. Pedro's plan: quick AO V1 history + status, the routing/quality problems (a few graphs), then the direction + remaining challenges. **CORRECTION (Pedro-confirmed):** the name is **"Peter"** (not "Peter Parker" — Otter hallucination); Bertrand 24:53 *"je vais le tenter avec Peter."* Pedro shares draft to **Bertrand + Yanira** ("Yann" = Yanira, Pedro-confirmed — NOT Ian). **Bertrand leaves Thursday after 12h next week (it's his birthday)** → get the draft to him before then.
