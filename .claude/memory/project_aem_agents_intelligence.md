...
> ## ▶️ RESUME HERE — left off **2026-08-10** (the owners call day itself; three decisions now stand, and the reporting axis was settled by publishing on the rejected axis with its error rate attached)
>
> **🔴 THE ONE NUMBER FOR MONDAY 08-10: the announcement bar produces 0 skills today, 4 if warnings are tolerated, on the 87 user-visible.** But **13 skills fail only disambiguation**, which is minutes of writing each → **17 announceable with no migration at all.** Full funnel in the **2026-08-06/07 block, archived to shard `..._ARCHIVE_2026-W32.md` on 08-10** (grep `AUDITS WENT PUBLIC` there). ⚠️ **Whether the 08-10 call happened, and what came out of it, is UNCAPTURED — ask before assuming either way.**
>
> **Three decisions now stand.** 08-06 — GA is two milestones, release belongs to the teams, the announcement is Pedro's. 08-07 — the checklist is the announcement bar, not a release gate. 08-07 — agent-cut numbers publish on the **skill name** with the measured error rate footnoted, because Rubin cannot log the plugin axis PR15 chose. **PR15's plugin rooting stays the record of record; the name is what gets published.**
>
> **🔴 CORRECTED 08-10 — the release mechanism is the skill's `lifecycle` field, NOT a plugin move between marketplace files.** Carsten put the plugin/marketplace work explicitly post-GA. Detail in the decision box below. **This puts `lifecycle` on the critical path of release, not only of vocabulary, which makes the `skill_inclusion_policy` finding a release argument.**
>
> **🔴 Bertrand is on PTO until AFTER 2026-08-24** — he misses 08-10, the 08-14 sign-offs, the bug bash and the GA. Heads-up drafted, two slots still blank (AI Ethics fallback, escalation cover). See the 08-07/10 block.
>
...
>
> **🔴 Bertrand is on PTO and returns AFTER 2026-08-24.** He misses the 08-10 owners call, the 08-14 sign-offs, the 08-17→21 bug bash **and the GA itself**. Pedro drafted a heads-up (his own draft, Claude critiqued). Two slots deliberately left blank because only he can fill them — **what he does if the AI Ethics sign-off on EPA is not there by 08-21**, and **who covers the escalation path to Loni while Bertrand is out**. 🔑 **The second one is the real gap: Bertrand is Pedro's channel upward, and the 08-24 announcement is a portfolio moment with that channel closed.**
>
