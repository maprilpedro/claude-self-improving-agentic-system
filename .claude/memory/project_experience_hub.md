...
> **🔴 ACTUAL MEETING HELD 2026-06-05 16h — "pas mal" with Guliz BUT EW-no-AEM-licence bombshell from Bertrand.** Pedro's read: good alignment with Guliz Sicotte on the EH = entry point recommendation. BUT Bertrand revealed: **EW does NOT require an AEM licence** → EW serves non-AEM customers. This crashes "one entry, two depths" (C1 resolution) for the non-AEM segment: EH→EW hub-and-spoke works only for AEM customers. Non-AEM EW users have no EH to enter from → EW must have its own home for that segment. "One entry, two depths" still valid and confirmed for AEM customers. **Pending:** what the right model is for non-AEM EW users, and whether this changes EH's scope or priorities. **Transcript NOT YET INGESTED:** `Agent Owner Alignement/20260605 - ExpHub, WE, Skills design alignement .md` — attempted multiple times, session ran out of context. **Next session:** ingest this transcript to capture the full meeting resolution.
