...
2. **Contribution model / UX AI Framework** — FULL ALIGNMENT (April 1). Pedro + Eugene + Sorin independently converged on the same model. Sorin's framing: "UX AI framework" — EH engineering as enabler, not bottleneck. He was already drafting this; it's the main focus for the 2 open reqs (UX + AI roles). Email thread exists between Sorin and Eugene — Pedro needs to be added. Success: 10 widgets EOY, start with 1-2 internal teams, expand to external customers. Scope: widgets first, pages later. Quality gate: start manual (Slack), automate with AI/Claude tooling.
