...
> - **🔴 Brian Chaikelson has now asked the same EH question three times — 08-05, 08-11, and by DM 08-12 06:05 with a clock on it** (*"I need to know for a GTM update I'm doing tomorrow (Wed)"*): what is still in H2 for Experience Hub (extensions, personalization revamp). **EH-side and still unanswered.**
...
> - **⏳ STILL OWED BY PEDRO: the go-to-market half + monthly release notes.** Ramon asked twice on 07-24 (*"still waiting for my documentation draft"* / *"is the go to marketing / monthly release notes prepared for Security Health GA?"*), Pedro answered *"That is on my todo P1"*. He also proposed a sync on Pedro's return from PTO. **Unknown to memory: whether the announcement was ever posted and whether that sync happened. Ask before drafting anything.**
