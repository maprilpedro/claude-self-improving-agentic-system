...
## 2026-09-11 — The shell calendar that gates the AEM panel

Rodson Clavel, `C0BCKG35NFP` 09-11 01:45: releasing the Coworker Panel for an app needs a Unified Shell release, and "there are only 2 more releases for Unified Shell (Sept 17 and Sept 24). After that shell will be in RCP for the rest of the year." Coworker-specific changes are exempt from RCP; the shell itself is not. Status 09-12 (Tim Lynn): Workfront and Target on; AEM, AJO, CJA "in the works"; AJO targets 09-24 (Gael Mouello signed off 09-14). Workfront no longer needs the URL params (Rodson 09-14). The AEM plan built on this: announce 09-24, TBYB activation the week after, SKU 10-05 (Pedro, 09-11). The custom-domain case (e.g. `author.nike.com`) is not solved by the Ethos wildcard fix and lands "in January" (Mark Doten 09-11); Mikaela's shell-domain render is the durable path.
