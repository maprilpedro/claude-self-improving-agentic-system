...
- **2026-07-06 (Mon), BEFORE Angela** — two checks to close from the doc read: (1) region — dataset homed in AEP AI Quality **VA7** org vs Ian's in-region hard requirement (Victor claims all regions); (2) reconcile Victor's AIQ-Events-Dataset door vs Ian's "DaaS Next + DPaaS single source" door — same object or two paths? (= the Ian ping drafted 07-03).
...
- **2026-07-06 (Mon, BEFORE Angela)** — Ian Boston reply on the two-doors + VA7 ping (**SENT 07-03**; two-doors question AIQ-Events-Dataset vs DaaS Next + DPaaS, VA7 region check, option-B scope line). If no reply by Angela time, raise the two checks in the Angela meeting directly.
