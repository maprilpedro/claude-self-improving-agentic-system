...
- 📏 **OBO HAS THREE LAYERS AND ONLY THE FIRST IS DONE (JIRA-verified 2026-08-11).** (1) **Server implements the exchange** — `AEMAGT-1361` Closed 08-06 (Tanju, One AEM MCP) and `AEMAGT-2429` "OBO with Discovery / Context Service" Closed 08-06 (Tanju). (2) **The IMS client is allowlisted for OBO exchange** — `GRANITE-67733` `aem-discovery-agent-obo` Resolved 07-01 (Waldemar Rokita); `IDOPS-31921` "Allowlist Client IDs for OBO exchange" Done 08-09. 🔴 **`GRANITE-71254` "Allowlist new client id for AEM Brand Governance Agent" is New and UNASSIGNED since 2026-07-30, never touched.** (3) **Downstream tools accept the new client_id** — 🔴 **`AEMAGT-2343` "Verify aem-shift-left MCP tools accept the new OBO client_id (`gpt_power_client`) before OBO re-enable", Gilles Knobloch, New and untouched since 2026-07-15.** 🔑 **So layer 1 is Tanju's and it shipped; layers 2 and 3 are per-agent-team and two of them have not moved in 12 and 27 days.**
- 🧠 **DEDUCTION, not confirmed.** What Carsten is untangling is probably layer 2, the **client-id allowlisting**, not the token exchange. It fits: Ian Boston blocks allowlisting on the AEM fleet (*"all traffic must go via the service proxy"*, `GRANITE-70593`), which would explain why `GRANITE-71254` sits unassigned. **And it would make `AEMAGT-2435` (Coworker cannot discover Brand Governance via One AEM MCP) downstream of the same blockage** — same agent, both stuck, and it reconciles Gerald saying the config is present with Philippe saying it is still broken. ⚠️ **Ask Carsten rather than assert this.**
