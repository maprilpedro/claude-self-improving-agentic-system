...
- **🔴 Settle the axis first. Rubin cannot group by plugin** (*"we still did not get that logging data"*), and the only fallback is the **skill name — the axis PR15 disqualified that same morning** (inferred, 67 of 88, disagrees with declared `domain:` on 14 of 18). **Two exits: get `plugin` logged, or publish on the name with the error rate in a footnote. Choosing neither = the substrate overrules the decision silently.**
