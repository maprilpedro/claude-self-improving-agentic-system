...
- 🆕🔴 **YANIRA IS BLOCKED ON EXACTLY THIS AND SHE WROTE TO PEDRO AT 2026-08-15 03:32** (DM `D018PT00155` → https://adobe.enterprise.slack.com/archives/D018PT00155/p1786757531908129). She has assigned **no author roles** because she does not know **which program**, **which environment** (*"Assuming Prod but someone needs to confirm"*, she wants Florin Florescu) or **which AEM role**. Done so far — a user group `AEM Agents on Coworker - GA Bug Bash - Authors`, plus **the CM Developer Role, required for EDA skills per the XL League doc**. She opened a thread to Florin → https://adobe.enterprise.slack.com/archives/C0BARAMM89Z/p1786714657830989?thread_ts=1786714378.824629&cid=C0BARAMM89Z. 🔑 **So the env question is not an architecture debate for Monday — it is an access blocker on every participant, and Pedro is the one who can answer it.** ⚠️ The CM Developer Role also kills the stage route independently: EDA needs Cloud Manager and CM is stage-bound.
- 🔑 **AND LENARD SAID THE CONSTRAINT OUT LOUD ON 08-14, before the merge:** *"In the first step I can include only local … and dev + stage, but then **the testing can be done only on experience-stage**."* ⚠️ Quote captured from Slack search context; **permalink not resolved — find it in the 08-14 manifest thread before requoting.**
...
- 🔴✅🔑 **SETTLED BY EXPERIMENT, 2026-08-15 20:57 — A STAGE COWORKER SESSION CANNOT REACH SHOWCASE'S AEM. `401 invalid OAuth token`.** Feeding `/aem-assets-discovery` the real Showcase stage-author URL `https://author-p149891-e1546482.adobeaemcloud.com` returned, verbatim: *"the AEM environments at `author-p149891-*` belong to a **different IMS org** than the one your session is authenticated against (`3C876885616965D90A494029@AdobeOrg`). The token issued for your current org is rejected by that host."* → **The cross-plane question is closed, and it took three minutes instead of an AO-ops round trip.** 🔑 **Note the gate is the IMS org on the AEM API auth, a different mechanism from the Coworker segment file, and both point the same way.** 🆕 CM Gandalf's org id is `3C876885616965D90A494029@AdobeOrg`.
- 🔴🔑🔑 **THE PATTERN OF THE WHOLE EVENING, AND IT IS THE MOST USEFUL THING TO CARRY INTO TUESDAY — EVERY SINGLE WALL WAS AN ENTITLEMENT, NEVER A SKILL AND NEVER A CONFIG.** Four in ninety minutes: `Failed to authenticate with Adobe ID` on the Showcase author (org admin, not in a product profile) · `401 invalid OAuth token` cross-org · `no environments provisioned` on CM Gandalf · and on `AEM Sites Engineering`, **`403 "Profile is not valid" (code 403025)`** from Cloud Manager, meaning the org or the user is not entitled or the product profile is unassigned. → **The dominant failure mode for 30 bug bashers on Tuesday will be product profiles, not skills. If that is not fixed in advance, the bash measures provisioning and reports it as skill quality**, and the 08-20 confidence declaration inherits the error. 🔑 **This is the strongest argument yet for answering Yanira before Monday, and it is now evidence rather than caution.**
- 🔑🔑 **→ SO THE BUG BASH RUNS ON THE PROD SHELL, AND THE MONDAY ASK IS NOW BACKED BY AN EXPERIMENT RATHER THAN A REPO READING.** Pedro can say he tested it and quote the 401. **The `prod/cx-coworker.yaml` `api_configs` port is the critical path, provable, not arguable.** ⏳ One branch left unexplored and deliberately so — **`AEM Sites Engineering` is in his stage org switcher and might carry provisioned AEM environments**, which would revive the stage route with a different target org. **It does not change the decision**: switching the bash target three days out, with ~30 people needing stage identities and Showcase chosen for its content, is not happening.
- ✅🔑🔑🔑 **THE CHAIN RUNS END TO END ON STAGE — FIRST REAL EXERCISE OF THE MERGED PLUGIN SET, 2026-08-15 20:50, BY PEDRO ALONE.** `/aem-cloudmanager-program-management` reported *"Loaded Aem Cloudmanager Program Management skill and **made 1 API call**"* and returned **13 real programs with IDs, status, type and solutions (9 Ready, 4 Failed)**, with Copy and CSV. **Manifest → skill load → auth → `cm` api_config → live API, all working.**
- ⚠️ **`correct:` 20:56 — THIS WAS BANKED AS "THE FIRST REAL BUG" AND IT IS PROBABLY NOT A BUG.** `/aem-assets-discovery` returned *"No AEM author repositories are available for your organization through the discovery service"* on an org showing 13 Cloud Manager programs. **Ten minutes later `/aem-cloudmanager-environment-management` on program `135659` answered *"has no environments provisioned. The environment list is empty."*** → **`CM Gandalf` is a Cloud Manager test org whose programs are shells, so an empty discovery result is the correct answer, not a defect.** 🔑 **What it does prove is that discovery ran and answered truthfully, and that a second skill independently corroborated the first.** ⚠️ **Lesson, on the night before a bug bash: a skill returning nothing is not a finding until the underlying data is checked** ([[feedback_dont_conflate_pattern_with_object]]).
- 🔑 **THIRD FINDING, AND IT TOUCHES COLE'S OPEN ASK. The skill offers a manual fallback — *"Provide a specific AEM author URL directly (e.g. `https://author-pXXXXX-eYYYYY.adobeaemcloud.com`)"*.** So **an instance can be named by hand when discovery fails**, which is a workaround today for the instance-picker problem Cole Connelly raised on 08-05. ⏳ **Two tests that follow, in order:** feed it a **CM Gandalf** author URL (isolates discovery from auth), then feed it **`author-p149891-e1546481`**, the Showcase prod-author, which would settle whether a stage shell can drive a prod AEM. ⚠️ **Expect the second to fail on identity** — a stage IMS token against a prod AEM — **and that failure is itself the answer.**
- ✅🔑🔑 **AND THE AEM SKILLS ARE LIVE ON THE STAGE COWORKER — CONFIRMED BY EYE 2026-08-15 20:48, ORG `CM Gandalf`.** `experience-stage.adobe.com/#/@cm-gandalf/coworker/…`, typing `/aem` returns **`/aem-assets-content-optimisation`, `/aem-assets-discovery`, `/aem-cloudmanager-environment-manage…`, `/aem-cloudmanager-pipeline-management`, `/aem-cloudmanager-pipeline-troubleshoot…`, `/aem-cloudmanager-program-management`** and the list scrolls further. → **The Friday merge is live and invocable.** ⚠️ **This proves the manifest loads, NOT that the skills reach a real AEM.** The API path is only proven by running one. 🔑 **If it runs end to end, the `prod/cx-coworker.yaml` `api_configs` port stops being a Tuesday blocker and becomes a GA-day one** — still needed for 08-24, not for the bash. ⏳ Still open, and it is now the only thing gating path B: **can ~30 people across seven teams get stage identities by Tuesday.** ⚠️ Naming, his own lane: **`aem-assets-content-optimisation` is British spelling** while the agent is Content Optimization.
- 🆕🟢🔑 **PATH B IS ALIVE — PEDRO HAS A STAGE IDENTITY AND COWORKER IS IN THE STAGE LAUNCHER (2026-08-15 20:47).** `experience-stage.adobe.com`, badge **Staging**, `Overrides on`, and an org switcher with **eight stage orgs** — `Adobe.com Stage`, **`AEM Sites Engineering`**, `AEM Trust Center 1`, `CM Gandalf` (current), `CM Gandalf DEV`, `Content AI Service - Stage`, `Evergreen Release Registry`, `Neal's ETLA Express Firefly`. Quick access shows **Coworker** and **Experience Manager**, both flagged New. → **The stage shell is reachable by him, so the bug-bash fork is genuinely two-sided, not one-sided as concluded earlier this evening.** ⚠️ **The org's presence in the launcher does not prove the AEM plugins load for it** — that depends on the segment the org sits in. **Clicking Coworker and asking an AEM question settles it, and would also be the first exercise of the 15 unverified plugins.** ⏳ Open: which of the eight carries an AEM program with real content (`AEM Sites Engineering` is the likeliest), and whether ~30 people across seven teams can get stage accounts by Tuesday.
- 🆕🟢 **THE "WHICH PROGRAM" QUESTION IS DOWN TO TWO CANDIDATES, NOT EIGHTEEN — Pedro read the Admin Console himself 2026-08-15 18:46, and he is admin on the org.** The 18 AEM CS instances resolve to: **`AEM Showcase Program`** (prod-author **2,084 users** / prod-publish 611 / prod-delivery 273, plus a stage set at 2,074) — the live SC demo program, real content, and **the exact org Corey's 08-06 *"we also use it for some external work"* objection was about**; and **`AEM Showcase EDA CxCowork-2`**, a **purpose-built Coworker program with a full dev/stage/prod author+publish set** (prod-author **11 users**), preceded by a `…cxcowork-2-1-dev-to-delete-…` pair, so somebody has already iterated on it. Plus two older `EDA-aem-showcase---eda-dev-*` instances (26 / 9 users) and org-wide **Cloud Manager (1,464 users)**, which is where Yanira's CM Developer Role sits — org-level, not per-program, so that half is fine.
- 🔑 **THE TWO IDs YANIRA IS BLOCKED ON EXIST AND PEDRO HAS THEM — CxCowork-2 prod-author is `Program 210760` / `Environment 2207539`.** Its four AEM product profiles are **AEM Administrators (5 entitled) · AEM Reporters (2) · AEM Sites Content Managers (2) · AEM Users (2)** = the 11 users on the instance list, i.e. **effectively nobody is in it**. **That profile list is also the answer to her third question** — bug bashers need `AEM Users` at minimum and `AEM Sites Content Managers` to create or edit. Pedro can create profiles there, so he is product admin.
- 🔴🔑 **THE BUG BASH FAILURE MODE WAS REPRODUCED LIVE ON 2026-08-15, BY PEDRO, ON HIMSELF.** He opened `author-p210760-e2207539` and got **`Failed to authenticate with Adobe ID`** — as **org admin on Showcase**. **Admin rights are not access: you reach an AEM environment only by being in one of its product profiles, and that environment entitles 11 people.** → **Every bug basher gets that same red box on Tuesday unless Yanira's group is attached to a product profile on the chosen environment.** That is exactly what she is blocked on, and it is now demonstrated rather than argued. ⚠️ Second, cheaper cause to rule out first — a browser signed into a different Adobe profile.
- ⚠️ **Admin Console shows entitlements, never content.** The content check is the author instance itself, `https://author-p210760-e2207539.adobeaemcloud.com` (which matches the skills' `host_allowlist: adobeaemcloud.com`), or Cloud Manager.
- ✅ **FORK RESOLVED 2026-08-15 BY LOOKING — CANDIDATE B IS DEAD.** `210760 / 2207539` prod-author has **zero assets** (`/content/dam` holds one empty `Projects` folder, "1 of 1") and **five stub Sites trees** — `campaigns`, `outbound-marketing`, `caid`, **`bugbash-wf-test`** and **`WF Test Page`**. **Discovery, EPA and Content Optimization would have nothing to act on**, so their feedback would read as skill failure. 🔎 **`bugbash-wf-test` + `WF Test Page` look like Workfront**, which would mean this rig was built for a WF/Coworker cross-test rather than for AEM's own bash. ⚠️ Inference from names only.
- ✅🔑 **ENVIRONMENT FOUND AND VERIFIED BY EYE, 2026-08-15 19:33 — `Program 149891` / `Environment 1546482`, badge `stage`, i.e. `aem-showcase-stage-author`.** https://author-p149891-e1546482.adobeaemcloud.com — **Sites** carries `aemshowcase` (with a `us` branch), **`Frescopa`**, `Company`, `Campaigns`, `Outbound Marketing`, `Test Site`; **Assets carries 19 folders of real content** including `Frescopa`, `MegaFormsDemo` (so **Forms has matter here**), three Summit-2026 sets, `hydrated-assets`, `generated-experiences` and two `assets-identity-service` repos. **Eight product profiles at 255-266 entitled users each** — Administrators, Assets Collaborator, Assets Power, Forms Content Managers, Forms Developers, Reporters, Sites Content Managers, Users. **This is the environment. Yanira's three answers are `149891` / `1546482` / `AEM Users` + `AEM Sites Content Managers` (+ `AEM Assets Collaborator Users` for asset work).**
- 🔴 **ONE UNKNOWN STILL GATES IT, AND IT IS THE SAME UNKNOWN AS THE MANIFEST ONE.** `api_configs.aem` is `kind: dynamic`, so the AEM host arrives at call time and the instance list comes from **`AEM_DISCOVERY_URL`**. **Whether a Showcase user gets `stage-author` offered on the prod AO deployment is therefore exactly the AO-ops question already open** — and it is also Cole Connelly's unanswered instance-picker ask. 🔑 **So Monday is ONE message with two questions, not three: who ports `api_configs` to `prod/cx-coworker.yaml` by COB, and what does `AEM_DISCOVERY_URL` enumerate on prod for a Showcase user.** ✅ **Both env IDs are now known — `149891 / 1546482` = stage-author, `149891 / 1546481` = prod-author** (same eight profiles, 256-270 entitled users each, so the same ~260 people are on both). **Taking prod-author removes the `AEM_DISCOVERY_URL` unknown from the bug bash entirely and leaves Monday with one question instead of two.**
- 🔑 **→ THE TARGET IS THE MAIN `AEM Showcase Program`, AND PEDRO'S STAGE INSTINCT WAS RIGHT ON THE WRONG AXIS.** `aem-showcase-**stage**-author` carries **2,074 users against prod-author's 2,084**, so it reads as a near-clone with the same people. **AEM-stage gets real content without thirty people writing test pages into the live SC demo prod, and it does NOT drag the AO shell to stage** — `api_configs.aem` is `kind: dynamic` with `host_allowlist: adobeaemcloud.com`, which both environments match. **The stage he was looking for is the AEM environment, not the AO deployment.** ⏳ **Two checks before committing to it:** content parity (open its Sites tree) and whether Coworker's AEM instance selection will actually offer it — **that second one is Cole Connelly's unanswered instance-picker ask**.
- ⚠️ **DO NOT READ THESE `-dev/-stage/-prod` SUFFIXES AS THE AO SHELL.** They are **AEM program environments** (Cloud Manager), the axis that is passed per call. **The AO deployment question is untouched by this list** ([[feedback_dont_conflate_pattern_with_object]]).
