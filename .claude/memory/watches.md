...
- **(pending Pedro)** — MCP business-reporting framework wiki **DRAFTED 07-03** → vault `AAI - Project Folder/MCP Business Reporting Framework (wiki draft) 2026-07-03.md`. Pedro reviews → publish to Confluence (strip the vault-only footer, add `wiki:` URL back per [[feedback_vault_copy_of_published_pages]]) → then the Christian+Jabran relance can point at it.
- **(pending Pedro)** — /reply draft to Christian+Jabran on the CloudWatch join (built 07-03, in session transcript + reproducible via /reply) — send as-is, or after the wiki ships. On send: update the CloudWatch watch to "nudged, awaiting reply".
