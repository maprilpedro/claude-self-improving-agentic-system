...
- 🐛🔑 **AND THE FIRST REAL BUG FELL OUT IMMEDIATELY. `/aem-assets-discovery` returned *"No AEM author repositories are available for your organization through the discovery service"*** on an org that has just listed **13 Cloud Manager programs, 9 of them Ready, several typed AEM Skyline with Sites and Assets**. **The discovery service ran and returned an empty set.** → **This is the `AEM_DISCOVERY_URL` question answered empirically instead of by asking AO ops, and it is exactly the class of finding the bug bash exists to produce.**
