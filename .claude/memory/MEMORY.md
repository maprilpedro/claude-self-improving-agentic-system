...
- [Coworker rail access](reference_coworker_rail_access.md) — **post-GA URL flag `?shell_aiChatEnabled=true` after `/ui`, not in the hash**; stage flags; the panel replaces the rail in place; two chat entry points in transition.
- [Coworker system-prompt blocks](reference_coworker_system_prompt_blocks.md) — **the answer to "what is Coworker's CLAUDE.md": `.ao/prompts/system/` block dirs, scoped per MANIFEST. 🔴 AEM has no prompt surface on the GA manifest.** The ask, the argument, and Shubham Lohiya as the target. Read before any "make Coworker behave like X".
- [AOv2 marketplace ≠ manifest](reference_aov2_marketplace_manifest.md) — convergence happens at the MANIFEST. **The AO developer reference settles it in one line: `known_marketplaces` "does not auto-install", `plugins` is what loads. Read the 2026-08-14 block first — doc URLs, CODEOWNERS, the two manifest copies, who writes the PR.** ⚠️ **never quote the 34.3% `when-to-use` figure — it is carried by excat (dev tooling). The customer-facing number is ~11% on 45 skills.** Read the 07-09 + 07-22 corrections before quoting any count.
- [Workday, the customer](reference_customer_workday.md) — **not the HR system.** Two AEMCS orgs, top-5 MCP user in June, the 09-15 Tina meeting with their engineer. Read before any Workday conversation.
...
- [🔴 Claude's own chat prose must be plain](feedback_claude_chat_prose_plain.md) — **no abstraction chains, one idea per sentence. And the knowledge base's vocabulary NEVER comes into chat — say what happened, don't name the pattern. Two failures, 08-03 and 08-06. Register is contagious.**
