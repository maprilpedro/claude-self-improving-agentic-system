...
If the file is **> ~2000 lines or > ~100K characters**, do not read it linearly. Spawn 3-5 parallel subagents on contiguous line ranges, each returning a structured extract (dated decisions, action items with owner, new stakeholders, verbatim quotes worth banking, risks). Synthesize from the structured extracts only. This is the `feedback_big_file_parallel_chunk_extract` memory rule — it keeps the main thread's context clean.
...
2. **Offer the trio** — do not auto-write it. After a meeting analysis the project's trio (Stakeholder Map / State of the Project / Questions for next 1-1) often wants updating. The EH/Sorin trio rule (`feedback_update_trio`) is EH-specific; the AAI trio is different. Name the specific trio for the owning project and ask if Pedro wants it synced, unless this transcript introduced a new stakeholder (then flag the Stakeholder Map gap directly).
