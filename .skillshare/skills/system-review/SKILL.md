...
- **Parked candidates** — for each, record the verdict in `knowledge/hypotheses/parked.md` (promoted/dropped/folded rows move to its History section): promoted (with where it landed), still parked (waiting for the Nth observation), or dropped as stale. "Independent" is the bar — two reps of the same event are one observation.
...
- **Index hygiene** — no orphaned hypothesis IDs; Sources Ingested rows carry source + date. INDEX.md has no hand-maintained entry counts and no per-session access log; git is the log.
- **Retrieval audit** — run `python3 scripts/retrieval_audit.py`. It reports which knowledge entries are never cited from working memory. Flag the most-stale never-cited entries to Pedro — his call per entry: still believed (keep), superseded (mark outdated with reasoning), or book-import that never applied to his reality (mark dormant). Never delete; the point is that unused knowledge stops hiding among the used.
- **Hot-file cap** — the belt-and-suspenders pass on the `/consolidate` Step-5 guard, in case a month of dense sessions slipped it. Run `python3 scripts/archive_memory.py --check` on all three hot files (`project_experience_hub.md`, `project_aem_agents_intelligence.md`, `.claude/state.md`); if a project file is over ~20K tokens, run the script (no `--check`) to shard old event blocks down + rebuild the archive index (20-24K residual after a dense week is tolerated — still loads in one Read); if `state.md` is over, move the oldest dated `## Notes` bullets into `.claude/state_ARCHIVE.md` by hand (never the Review log or the decisions tables). This is also the right moment to move a **living-reference section that has gone quiet** into `..._ARCHIVE_reference.md` by hand (the script only moves dated event blocks). Never delete — move + index.
