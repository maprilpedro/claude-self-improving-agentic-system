...
- **Index hygiene** — no orphaned hypothesis IDs; Sources Ingested rows carry source + date. (The Folder-Inventory entry-count column was retired 2026-07-01, the Access Log 2026-07-02 — don't reintroduce hand-maintained counts or per-session logs; git is the log.)
- **Retrieval audit (P6, 2026-07-02)** — run `python3 scripts/retrieval_audit.py`. It reports which knowledge entries are never cited from working memory (baseline 2026-07-02: 292 of 313 never cited). Flag the most-stale never-cited entries to Pedro — his call per entry: still believed (keep), superseded (mark outdated with reasoning), or book-import that never applied to his reality (mark dormant). Never delete; the point is that unused knowledge stops hiding among the used.
